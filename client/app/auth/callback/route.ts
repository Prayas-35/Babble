  ...
  if (code) {
    ...
    if (!error && data.session) {
      ...
      if (data.session.provider_token) {
        response.cookies.set("provider_token", data.session.provider_token, cookieOpts);
      }
      if (data.session.provider_refresh_token) {
        response.cookies.set("provider_refresh_token", data.session.provider_refresh_token, cookieOpts);
      }
